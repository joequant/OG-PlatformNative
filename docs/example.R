...
method_info <- new.env(hash=TRUE)

for (f in ls()) {
    if (!f == "f") {
        fn <- get(f)
        source_code <- paste(deparse(fn), collapse="\n")
        #        if (!class(fn) == "character") {
        #            print(formals(fn))
        #        }
        assign(f, source_code, env=method_info)
    }
}
data_file <- file("dexy--opengamma-methods.json", "w")
writeLines(toJSON(as.list(method_info)), data_file)
close(data_file)
