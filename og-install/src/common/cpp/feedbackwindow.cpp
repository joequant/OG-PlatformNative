...
void CFeedbackWindow::SetAsParent (HWND hwndChild) {
	if (hwndChild == m_hwnd) {
		return;
	}
	SetWindowLongPtr (hwndChild, GWLP_HWNDPARENT, (LONG_PTR)m_hwnd);
	SetWindowPos (hwndChild, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
	SetWindowPos (hwndChild, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
}
